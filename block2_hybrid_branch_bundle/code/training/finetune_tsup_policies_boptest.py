...
def build_scratch_ppo(env: gym.Env, *, learning_rate: float, ppo_epochs: int, seed: int) -> PPO:
    return PPO(
        "MlpPolicy",
        env,
...
class BOPTESTDirectTSupEnv(gym.Env):
    ...
    def _compute_reward(self, info: dict[str, float], action: np.ndarray) -> float:
        ...
        center_error = t_zone - T_CENTER
        abs_center_error = abs(center_error)
        if t_zone < T_LOW:
            comfort_violation = T_LOW - t_zone
        elif t_zone > T_HIGH:
            comfort_violation = t_zone - T_HIGH
        else:
        ...
        if comfort_violation <= 0.0:
            dist_norm = min(abs_center_error / max(0.5 * (T_HIGH - T_LOW), 1e-6), 1.0)
            r_track = 0.40 - 0.25 * (dist_norm**2)
            if abs_center_error <= 0.5:
                r_track += 0.35 * (1.0 - abs_center_error / 0.5)
        else:
            if t_zone < T_LOW:
...
def finetune_one_agent(
    ...
    init_from_scratch = bool(agent == "thermostatic" and args.scratch_thermostatic)
    model_obs_dim = EXTENDED_TSUP_OBS_DIM if init_from_scratch else validate_model_obs_dim(model_path)
    ...
    print(f"{'=' * 72}")
    print(f"Init mode:    {'scratch' if init_from_scratch else 'warm_start'}")
    print(f"Model:        {model_path if not init_from_scratch else '[new PPO]'}")
    ...
    save_suffix = "scratch" if init_from_scratch else "finetuned"
    save_path = out_dir / f"{agent}_step{args.step_sec}_{save_suffix}"
...
def main() -> None:
    ...
    parser.add_argument("--scratch-thermostatic", action="store_true")
    ...
    snapshot = {
        ...
        "seed": args.seed,
        "scratch_thermostatic": args.scratch_thermostatic,
        "weather_csv": weather_csv,
    ...
    print(f"Step sec:      {args.step_sec}")
    print(f"Episode days:  {args.episode_days}")
    print(f"Scratch therm: {args.scratch_thermostatic}")
