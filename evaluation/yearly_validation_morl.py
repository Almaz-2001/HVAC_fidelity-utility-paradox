...
ADVANCE_TIMEOUT = 60
HTTP_RETRIES = 3
BASIC_OBS_DIM = BASIC_TSUP_OBS_DIM
...
def advance(testid, actions):
    data = boptest_request("POST", f"/advance/{testid}", actions, timeout=ADVANCE_TIMEOUT, retries=HTTP_RETRIES)
...
def make_obs(
    ...
    if obs_dim == BASIC_OBS_DIM:
    ...
    elif obs_dim in SUPPORTED_TSUP_OBS_DIMS:
        obs = build_tsup_obs(
            t_c,
            co2,
            p_total,
        ...
        raise RuntimeError(
            ...
            f"Supported: {BASIC_OBS_DIM}, {sorted(SUPPORTED_TSUP_OBS_DIMS)}."
        )
...
def run_scenario(
    ...
    print("  Priming first advance...")
    payload = advance(testid, {})
    print("  First advance complete")

    prev_action = np.zeros(2, dtype=np.float32)
    obs, t_zone, p_total, t_amb = make_obs(
        payload,
        prev_action,
        None,
        weather,
...
def main() -> None:
    ...
    parser.add_argument("--delta-feature-mode", choices=sorted(SUPPORTED_DELTA_FEATURE_MODES), default="causal_smooth")
    ...
    obs_shape = data.get("observation_space", {}).get("_shape", [BASIC_OBS_DIM])
    obs_dim = obs_shape[0] if isinstance(obs_shape, list) else obs_shape
    if obs_dim != BASIC_OBS_DIM and obs_dim not in SUPPORTED_TSUP_OBS_DIMS:
        raise RuntimeError(
            f"Model observation dim is {obs_dim}, but MORL yearly validation expects "
            f"{BASIC_OBS_DIM} or {sorted(SUPPORTED_TSUP_OBS_DIMS)}. "
    ...
    for name, start_time in SCENARIOS.items():
        results.append(
            run_scenario(
                ...
                int(obs_dim),
                args.obs_ablation,
                args.delta_feature_mode,
                args.t_zone_feature_mode,
                args.power_feature_mode,
            )
        )
