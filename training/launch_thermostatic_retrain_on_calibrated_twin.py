...
def build_command(extra_args: list[str]) -> list[str]:
    return [
        sys.executable,
        str(ROOT / "training" / "train_thermostatic.py"),
        "--surrogate-kind",
        "v35_calibrated",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Launch thermostatic PPO retraining on the calibrated 15-minute v3.5 direct-TSup surrogate."
    )
    _, extra_args = parser.parse_known_args()

    cmd = build_command(extra_args)
    print("Thermostatic retrain on 15-minute v35_calibrated")
    print("Command:")
