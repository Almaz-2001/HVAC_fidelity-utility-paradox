...
N_OBS = 17  # 5 physical + 4 time cyc + 5 forecast + 2 prev_action + 1 delta_t
...
class ThermostaticEnv(Env):
    ...
    def __init__(self, surrogate_path=None, dr_enabled=True):
        super().__init__()
        ...
        cp = torch.load(surrogate_path, map_location="cpu", weights_only=False)
        self.model = RCNeuralODEv2(hidden_dim=cp.get("hidden_dim", 64))
...
def make_env(rank, seed=42):
    def _init():
        env = ThermostaticEnv(dr_enabled=True)
...
def main():
    ...
    print(f"Obs: {N_OBS}F [T_zone, CO2, P_total, TSup_prev, T_amb + time/forecast/history]")
    print(f"Forecast horizons: {FORECAST_HORIZONS}h")
    print(f"TSup range: [{T_SUPPLY_LOW}, {T_SUPPLY_HIGH}]C")
    print(f"Surrogate: {SURROGATE_PATH_DEFAULT}")
    ...
    model = PPO(
        ...
        verbose=1,
        tensorboard_log="./logs/thermostatic_v3/",
    )
    ...
    model.save("./models/ppo_thermostatic")
    vec_env.close()
    print(f"\nDone: {elapsed:.1f} min -> models/ppo_thermostatic.zip")

    print(f"\n{'=' * 60}")
    print("SURROGATE EVAL")
    print(f"{'=' * 60}")
    env = ThermostaticEnv(dr_enabled=False)
    env = TimeLimit(env, max_episode_steps=336)
